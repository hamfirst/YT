...
export module YT:MultiProducerSingleConsumer;
...
namespace YT
{
    ...
    {
        static_assert(Capacity > 0, "MultiProducerSingleConsumer requires Capacity > 0");

    public:
        MultiProducerSingleConsumer() noexcept
        {
        ...
        MultiProducerSingleConsumer(const MultiProducerSingleConsumer &) = delete;
        MultiProducerSingleConsumer(MultiProducerSingleConsumer &&) = delete;

        MultiProducerSingleConsumer & operator = (const MultiProducerSingleConsumer &) = delete;
        MultiProducerSingleConsumer & operator = (MultiProducerSingleConsumer &&) = delete;
        ...
        /// Call from one consumer thread only (no CAS on dequeue; multiple consumers are unsupported).
        [[nodiscard]] bool TryDequeue(T & out) noexcept(std::is_nothrow_move_assignable_v<T> &&
                                                        std::is_nothrow_move_constructible_v<T>)
        {
            const std::size_t position = m_DequeuePos.load(std::memory_order_relaxed);
            Slot & slot = m_Slots[position % Capacity];
            const std::size_t sequence = slot.m_Sequence.load(std::memory_order_acquire);
            ...
            m_DequeuePos.store(position + 1, std::memory_order_release);
            return true;
        }
