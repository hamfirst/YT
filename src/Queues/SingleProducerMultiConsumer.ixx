...
export module YT:SingleProducerMultiConsumer;

namespace YT
{
    export template <typename T, std::size_t Capacity>
    class SingleProducerMultiConsumer
    {
        static_assert(Capacity > 0, "SingleProducerMultiConsumer requires Capacity > 0");

    public:
        SingleProducerMultiConsumer() noexcept
        {
        ...
        SingleProducerMultiConsumer(const SingleProducerMultiConsumer &) = delete;
        SingleProducerMultiConsumer(SingleProducerMultiConsumer &&) = delete;

        SingleProducerMultiConsumer & operator = (const SingleProducerMultiConsumer &) = delete;
        SingleProducerMultiConsumer & operator = (SingleProducerMultiConsumer &&) = delete;
        ...
        {
            const std::size_t position = m_EnqueuePos.load(std::memory_order_relaxed);
            ...
            m_EnqueuePos.store(position + 1, std::memory_order_relaxed);
            return true;
        }
